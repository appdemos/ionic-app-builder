		...
		$(function() {
			$('#elfinder').elfinder(
				// 1st Arg - options
				{
					...
					// Connector URL
					url : 'php/connector.minimal.php?type=<?php echo $_GET['type'];?>',
					...
					getFileCallback : function(file,fm) {
					   ...
					   <?php
                            ...
                            if(isset($_GET['field'])){
                                echo '
                                    var filePath = "output/' . $_SESSION['PROJECT']['app']['prefix'] . '/www/data/" +  file.path;
                                    var file_url = filePath.replace(/\\\/g,"/") ;    
                                    win = (window.opener ? window.opener : window.parent);                        
                                    $(win.document).find("#'.$_GET['field'].'").val(file_url);
                                    win.tinyMCE.activeEditor.windowManager.close();                                
                                ';
                            } else{
                                echo '                       
                                    var filePath = "/www/data/" +  file.path;
                                    var file_url = filePath.replace(/\\\/g,"/") ;    
                                    window.opener.KCFinder.callBack( file_url );
                                    window.close();                
                                ';
                            }                      
                       ?>
...
<body>
	<div id="elfinder" style="height: 100%; width: 100%;"></div>
</body>
</html>
